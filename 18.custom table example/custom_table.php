<?php
/*
Plugin Name: 18.Custom table example good
...
register_activation_hook(__FILE__, 'custom_table_example_install'); //The register_activation_hook() function will make sure that it will automatically
// create a database table using the code inside the function custom_table_example_install
...
class Custom_Table_Example_List_Table extends WP_List_Table
{
    ...
    function column_name($item)
    {
        ...
        $actions = array(
            ...
            'delete2' => sprintf('<a href="?page=%s&action=delete2&id=%s">%s</a>', $_REQUEST['page'], $item['id'], __('Delete2', 'custom_table_example')),
            );
...
function custom_table_example_persons_page_handler()
{
    ...
    if ('delete2' === $table->current_action()) {
        $message = '<div class="updated below-h2" id="message"><p>' . sprintf(__('Items deleted by delete2 options: %d', 'custom_table_example'), count($_REQUEST['id'])) . '</p></div>';
    }
    ?>
<div class="wrap">
    ...
    <h2><?php _e('Persons', 'custom_table_example')?>
        <a class="add-new-h2" href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=persons_form');?>"><?php _e('Add new', 'custom_table_example')?></a>
    </h2>
    <?php echo $message; ?>
...
function custom_table_example_persons_form_meta_box_handler($item)
{
    ?>
<!--*************************************
show form when we click add new
*****************************************
   -->
